...
from .models import Usuario, Rol, Negocio, Producto, Pedido, DetallePedido, HistorialCambioProducto, PerfilRepartidor, Pago, Calificacion, Incidencia
...
class RegisterRepartidorSerializer(serializers.Serializer):
    ...
    telefono = serializers.CharField(max_length=20, validators=[validar_telefono_peruano])
...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    telefono   = serializers.CharField(validators=[validar_telefono_peruano])
...
class PagoSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = [
            'numero_transaccion', 'fecha',
            'comision_plataforma', 'monto_comercio', 'monto_repartidor',
        ]


class HistorialPagoSerializer(serializers.ModelSerializer):
    """HU15 — una fila del historial de pagos, con la distribución calculada en HU14."""
    monto_total       = serializers.DecimalField(source='monto', max_digits=10, decimal_places=2, read_only=True)
    negocio_nombre    = serializers.CharField(source='pedido.negocio.nombre', read_only=True)
    ...
    class Meta:
        ...
        fields = [
            ...
            'comision_plataforma', 'monto_comercio', 'monto_repartidor',
            'negocio_nombre', 'repartidor_nombre',
        ]

    def get_repartidor_nombre(self, obj):
        repartidor = obj.pedido.repartidor
        if not repartidor:
            return None
        return f'{repartidor.nombre} {repartidor.apellido}'.strip()

class CalificacionSerializer(serializers.ModelSerializer):
    class Meta:
        model  = Calificacion
        fields = ['id', 'pedido', 'repartidor', 'estrellas', 'comentario', 'fecha']
        read_only_fields = ['pedido', 'repartidor', 'fecha']
...
class IncidenciaSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model  = Incidencia
        fields = [
            'id', 'pedido', 'tipo', 'descripcion', 'estado', 'respuesta',
            'cliente_nombre', 'created_at',
        ]
        read_only_fields = ['pedido', 'estado', 'respuesta', 'created_at']
...
class PedidoSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'detalles', 'created_at', 'calificacion', 'incidencia',
        ]
    ...
    def get_calificacion(self, obj):
        try:
            return CalificacionSerializer(obj.calificacion).data
        except Calificacion.DoesNotExist:
            return None

    def get_incidencia(self, obj):
        incidencia = obj.incidencias.order_by('-created_at').first()
        if not incidencia:
            return None
