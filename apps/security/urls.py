...
from .views import (
    ...
    RegisterRepartidorView, PedidosDisponiblesView, TomarPedidoView,
    DetallePedidoConPagoView, PagarPedidoView,
    CookieTokenRefreshView,
    CalificarPedidoView, CalificacionPedidoView, PromedioCalificacionesRepartidorView,
    HistorialPagosView,
    CrearIncidenciaView, IncidenciasListView, ResponderIncidenciaView,
    ReporteDiarioView, ReporteNegocioView,
...
urlpatterns = [
    ...
    # Pago
    path('pedidos/<int:pk>/detalle/', DetallePedidoConPagoView.as_view(), name='pedido-detalle'),
    path('pedidos/<int:pk>/pagar/',   PagarPedidoView.as_view(),          name='pagar-pedido'),
    # Repartidor
    path('pedidos/disponibles/', PedidosDisponiblesView.as_view(), name='pedidos-disponibles'),
    path('pedidos/<int:pk>/tomar/', TomarPedidoView.as_view(), name='tomar-pedido'),
    ...
    # Calificaciones
    path('pedidos/<int:pk>/calificar/',    CalificarPedidoView.as_view(),    name='calificar-pedido'),
    path('pedidos/<int:pk>/calificacion/', CalificacionPedidoView.as_view(), name='calificacion-pedido'),
    path('repartidor/calificaciones/promedio/', PromedioCalificacionesRepartidorView.as_view(), name='promedio-calificaciones-repartidor'),
    # Pagos — historial y distribución de ingresos (HU14 / HU15)
    path('pagos/', HistorialPagosView.as_view(), name='historial-pagos'),
    # Incidencias — registro y gestión de reclamos (HU16 / HU17)
    path('pedidos/<int:pk>/incidencias/', CrearIncidenciaView.as_view(),  name='crear-incidencia'),
    path('incidencias/',                  IncidenciasListView.as_view(), name='lista-incidencias'),
    path('incidencias/<int:pk>/responder/', ResponderIncidenciaView.as_view(), name='responder-incidencia'),
    # Reportes (HU18 / HU19)
    path('reportes/diario/', ReporteDiarioView.as_view(), name='reporte-diario'),
    path('negocio/reporte/', ReporteNegocioView.as_view(), name='reporte-negocio'),
    # Predicción de demanda (HU20)
    path('predicciones/demanda/', PrediccionDemandaView.as_view(), name='prediccion-demanda'),
]